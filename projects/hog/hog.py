...
def swine_align(player_score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    GCD = CD = 2
    while CD <= min(player_score,opponent_score):
        if player_score % CD == 0 and opponent_score % CD == 0:
            GCD = CD
        CD += 1
    if player_score > 0 and opponent_score > 0 and GCD >= 10:
        return True
    else:
        return False
    # END PROBLEM 4a
    ...
    def say(score0,score1):
        if who == 0:
            ...
            if current_high > running_high:
                print(f'{current_high} point(s)! The most yet for Player 0')
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    max_average_roll_num = 0
    max_average = 0

    for i in range(1,11):
        average = make_averaged(roll_dice,trials_count)
        current_average = average(i,dice)
        if max_average < current_average:
            max_average = current_average
            max_average_roll_num = i

    return max_average_roll_num
    # END PROBLEM 9
...
def final_strategy(score, opponent_score,cutoff=9,num_rolls=6):
    """This is a straty of Campbell,based on extra_turn_strategy
