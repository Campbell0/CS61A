...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def if_about(string):
        string = split(lower(remove_punctuation(string)))
        for i in topic:
            if i in string:
                return True
        return False
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    for w in valid_words:
        current_diff = diff_function(user_word,w,limit)
...
def report_progress(typed, prompt, user_id, send):
    ...
    for i in range(len(typed)):
        if typed[i] == prompt[i]:
            ...
            break
    progress = correct_nums / len(prompt)
...
def time_per_word(times_per_player, words):
    ...
    for i in range(len(times_per_player)):
        ...
        for j in range(len(words)):
            current_player.append(times_per_player[i][j+1] - times_per_player[i][j])
        times_player_used.append(current_player)
    return game(words,times_player_used)
    # END PROBLEM 9
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    fastest_word = [[] for i in player_indices]
    for w in word_indices:
        min_time = float('inf')
        for p in player_indices:
            if min_time > time(game,p,w):
                min_time = time(game,p,w)
                min_time_player = p
        fastest_word[min_time_player].append(word_at(game,w))
    return fastest_word
    # END PROBLEM 10
